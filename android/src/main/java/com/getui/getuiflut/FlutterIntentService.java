...
public class FlutterIntentService extends GTIntentService {
    ...
    private static final String SHARED_PREFERENCES_KEY = "com.getui.getuiflut";
    ...
    public static final String ACTION_REMOTE_MESSAGE =
            "com.getui.getuiflut.NOTIFICATION";
    public static final String EXTRA_REMOTE_MESSAGE = "notification";
    public static final String EXTRA_REMOTE_METHOD = "method";
    ...
    private static List<Map<String, Object>> backgroundMessageQueue =
    ...
    public static void onInitialized() {
        ...
        synchronized (backgroundMessageQueue) {
            ...
            Iterator<Map<String, Object>> i = backgroundMessageQueue.iterator();
            while (i.hasNext()) {
    ...
    public void onReceiveClientId(Context context, String clientid) {
        ...
        intent.putExtra(EXTRA_REMOTE_MESSAGE, clientid);
        intent.putExtra(EXTRA_REMOTE_METHOD, "onReceiveClientId");
    ...
    private static void executeDartCallbackInBackgroundIsolate(
            Context context, Map<String, Object> remoteMessage, final CountDownLatch latch) {
        ...
        args.put("message", remoteMessage);
    ...
    public void onReceiveMessageData(Context context, GTTransmitMessage msg) {
        ...
        if (isApplicationForeground(this)) {
            Intent intent = new Intent(ACTION_REMOTE_MESSAGE);
            intent.putExtra(EXTRA_REMOTE_MESSAGE, msg);
            intent.putExtra(EXTRA_REMOTE_METHOD, "onReceiveMessageData");
            LocalBroadcastManager.getInstance(this).sendBroadcast(intent);
        } else {
            // If background isolate is not running yet, put message in queue and it will be handled
            // when the isolate starts.
            if (!isIsolateRunning.get()) {
                backgroundMessageQueue.add(payload);
            } else {
                                ...
                                new Runnable() {
                                    ...
                                    public void run() {
                                        executeDartCallbackInBackgroundIsolate(
                                                FlutterIntentService.this, payload, latch);
                                    }
                                });
                try {
                    latch.await();
    ...
    public void onReceiveOnlineState(Context context, boolean b) {
        Intent intent = new Intent(ACTION_REMOTE_MESSAGE);
        intent.putExtra(EXTRA_REMOTE_MESSAGE, String.valueOf(b));
        intent.putExtra(EXTRA_REMOTE_METHOD, "onReceiveOnlineState");
        LocalBroadcastManager.getInstance(this).sendBroadcast(intent);
    }
    ...
    public void onReceiveCommandResult(Context context, GTCmdMessage gtCmdMessage) {
        Intent intent = new Intent(ACTION_REMOTE_MESSAGE);
        intent.putExtra(EXTRA_REMOTE_MESSAGE, gtCmdMessage.toString());
        intent.putExtra(EXTRA_REMOTE_METHOD, "onReceiveCommandResult");
        LocalBroadcastManager.getInstance(this).sendBroadcast(intent);
    }
    ...
    public void onNotificationMessageArrived(Context context, GTNotificationMessage message) {
        ...
        intent.putExtra(EXTRA_REMOTE_METHOD, "onNotificationMessageArrived");
        LocalBroadcastManager.getInstance(this).sendBroadcast(intent);
    }
    ...
    public void onNotificationMessageClicked(Context context, GTNotificationMessage message) {
        ...
        Intent intent = new Intent(ACTION_REMOTE_MESSAGE);
        intent.putExtra(EXTRA_REMOTE_MESSAGE, message);
        intent.putExtra(EXTRA_REMOTE_METHOD, "onNotificationMessageClicked");
        LocalBroadcastManager.getInstance(this).sendBroadcast(intent);
    }
}
